...
const globalHandlers: ErrorHandlerItems = {
	[HttpStatusCode.UNAUTHORIZED]: (axiosError: AxiosError) => {
		console.error('Un-authorized');
	},
	[HttpStatusCode.SERVER_ERROR]: (axiosError: AxiosError) => {
...
export function axiosErrorHandler(error: AxiosError, customHandlers?: ErrorHandlerItems) {
	...
	if (error.response) {
		...
		if (customHandlers) {
			...
			for (let item in customHandlers) {
				copied[item] = customHandlers[item];
			}

			// Process handlers
			if (copied.hasOwnProperty(error.response.status)) {
